...
exports.init = (appIns) => {
  ...
  if(!config.database.enabled){
    app.console.warn('[WARN] database has not enabled.');
  }else if(!config.database.dbname){
    app.console.warn('[WARN] no database specified, please check you database config.');
  }else {
    ...
    try {
      ...
      if(config.database.orm === 'sequelize'){
        ...
        app.model = app.context.model = modelModule;
      }else app.console.warn('[WARN] unsupported orm.');
