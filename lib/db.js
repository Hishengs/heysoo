...
exports.init = (appIns) => {
  ...
  if(!config.database.enabled){
    app.console.info('[warning] database has not enabled.');
  }else if(!config.database.dbname){
    app.console.info('[warning] no database specified, please check you database config.');
  }else {
    ...
    try {
      ...
      if(config.database.orm === 'sequelize'){
        ...
        app.model = app.context.model = modelModule;
      }else app.console.info('[warning] unsupported orm.');

      app.console.info('database init done.');
