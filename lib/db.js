...
exports.init = (appIns) => {
  ...
  if(!config.database.enabled){
    app.logger.info('[warning] database has not enabled.');
  }else if(!config.database.dbname){
    app.logger.info('[warning] no database specified, please check you database config.');
  }else {
    ...
    try {
      ...
      if(config.database.orm === 'sequelize'){
        ...
        app.model = app.context.model = modelModule;
      }else app.logger.info('[warning] unsupported orm.');

      app.logger.info('database init done.');
