...
module.exports = class Heysoo extends Koa {
  ...
  init() {
    ...
    if (this.config.logRequestTime) {
      this.use(async (ctx, next) => {
        const timestampIn = performance.now();
        this.console.mark(`before request: ${timestampIn.toFixed(3)}`);
        await next();
        const timestampOut = performance.now();
        const timestampGap = this.console.color.green(`+${(timestampOut - timestampIn).toFixed(3)} ms`);
        this.console.mark(`after  request: ${timestampOut.toFixed(3)} ${timestampGap}`);
      });
    }
  ...
  */
  async start(callback) {
    ...
    this.console.mark(`app is listening at ${this.config.host}:${this.config.port}`);
  ...
  */
  getPathOf (name) {
    ...
    } else if (['app', 'router'].includes(name)) {
