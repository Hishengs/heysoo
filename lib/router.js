...
module.exports.init = (app) => {
  ...
  class Router extends KoaRouter {
    ...
    constructor (opts = {}){
      super(opts);
      this.supportedMethods = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS', 'COPY', 'LINK', 'UNLINK', 'PURGE', 'LOCK', 'UNLOCK', 'PROPFIND', 'ALL'];
  ...
  app.use(async (ctx, next) => {
    ...
    // };
    router.currentRoute = ctx._matchedRoute;
    router.currentRouteName = ctx._matchedRouteName;
    await next();
  });
  ...
  try {
    ...
    // e.message = '[error] router init failed, please check you router config.\n' + e.message;
    // throw e;
    app.console.warn('[WARN] router init failed, please check you router config.');
  }
